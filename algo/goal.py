...
class Goal:
    def __init__(self, env_params, args, action_nums=200, act=None, cri=None):
        self.device = args.device
        self.type = args.graph_type
        self.G = nx.DiGraph()
        self.parts = args.graph_part
        ...
        if self.type == 'neighbor':
            ...
            for i in range(batch_size):
                ...
                if self.is_random:
                    ...
                    if group == 0 or group == 1:
                        idxs = candidate[group][i]
                    else:
                        idxs = list(set(group3) - set(group1[i]) - set(group2[i]))
                    states = [self.G.nodes[k]['state'] for k in idxs]
                    states_tensor = torch.tensor(states, dtype=torch.float32).to(self.device)
                    goals = curgoal[i].repeat(len(idxs), 1)
                    with torch.no_grad():
                        pi = self.actor(states_tensor, goals)
                        q_pi = self.critic(states_tensor, goals, pi)
                        node_idx = torch.argmax(q_pi).item()
                    newgoal.append(states[node_idx])
            ...
            if certainty_nums>2:
                partitions = certainty_nums // 3
                group1 = certainty_nodes[0:partitions]
                group2 = certainty_nodes[partitions: partitions * 2]
                group3 = certainty_nodes[partitions * 2:certainty_nums]
                candidate = [group1, group2, group3]
            else:
                candidate = [certainty_nodes,certainty_nodes,certainty_nodes]

            if not self.is_random:
                candidate = [list(itertools.chain.from_iterable(sub)) for sub in candidate]
            ...
            if self.is_random:
                ...
                states =[[self.G.nodes[k]['state'] for k in candidate[j]] for j in range(self.parts)]
                states_tensor = [torch.tensor(states[j], dtype=torch.float32).to(self.device) for j in range(self.parts)]

                for i in range(batch_size):
                    group = category[i].item()
                    goals = curgoal[i].repeat(len(states[group]), 1)
                    with torch.no_grad():
                        pi = self.actor(states_tensor[group], goals)
                        q_pi = self.critic(states_tensor[group], goals, pi)
                        node_idx = torch.argmax(q_pi).item()
                    newgoal.append(states[group][node_idx])
    ...
    def update(self, training_samples):
        ...
        if self.type == 'neighbor':
            ...
            if certainty_nums>2:
                ...
                group2 = certainty_nodes[partitions: partitions * 2]
                group3 = certainty_nodes[partitions * 2:certainty_nums]
                candidate = [list(itertools.chain.from_iterable(group1)), list(itertools.chain.from_iterable(group2)), list(itertools.chain.from_iterable(group3))]
            else:
