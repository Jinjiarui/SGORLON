...
from utils.networks import Qnet
from utils.replaybuffer import ReplayBuffer
from utils.her import HER
import random
from tensorboardX import SummaryWriter
import torch
from map.goal import Goal
from map.discretized import DiscretizedWrapper
...
class DQN:
    def __init__(self, args, env, env_params, test_env):
        ...
        self.discret_s = DiscretizedWrapper(n_bins=self.args.dis_s,low=self.env.observation_space['observation'].low,high=self.env.observation_space['observation'].high)
        self.discret_a = DiscretizedWrapper(n_bins=self.args.dis_a,low=self.env.action_space.low,high=self.env.action_space.high)
        self.goalgenerator = Goal(env_params)
        ...
        # her sampler
        self.her_module = HER(self.args.replay_strategy, self.args.replay_k, self.args.distance, self.goalgenerator)
        # create the replay buffer
        self.buffer = ReplayBuffer(self.env_params, self.args.buffer_size, self.her_module.sample_her_transitions)


    def init_qnets(self):
        self.Q_network = Qnet(self.env_params, self.args).to(self.device)
        self.targetQ_network = Qnet(self.env_params, self.args).to(self.device)

    def learn(self):
        for epoch in range(self.start_epoch, self.args.n_epochs):
            ep_obs, ep_ag, ep_g, ep_actions,ep_obs_d,ep_actions_d = [], [], [], [],[],[]
            observation = self.env.reset()
            obs = observation['observation']
            ag = observation['achieved_goal']
            g = observation['desired_goal']
            obs_d = self.discret_s.discrete(obs)
            for t in range(self.env_params['max_timesteps']):
                ...
                ep_actions_d.append(action_d.copy())
            ...
            ep_obs_d.append(obs_d.copy())
            ep_ag.append(ag.copy())
            self.goalgenerator.addgraph(ep_obs,ep_obs_d,ep_actions_d)
            ...
            if epoch % 10 == 0:
                ...
                torch.save([self.Q_network.state_dict()], self.model_path + '/q_model_' + str(epoch) + '.pt')
